...
class MockRollout(base_rollout.BaseRollout):
  ...
  Behaviors mocked:
    * Text Generation: Produces sequences of random dummy words. The length
        can be configured via `rollout_mock_length_distribution` in
        `RolloutConfig`.
        Supported modes:
        ...
        Users can optionally override the shape by specifying
        `rollout_mock_length_mean`
        and `rollout_mock_length_std`.
  ...
  def _get_target_length(
      self, rollout_config: base_rollout.RolloutConfig, max_tokens: int
  ) -> int:
    """Calculates the target length for generation based on distribution config."""
    distribution = rollout_config.rollout_mock_length_distribution
    mu = rollout_config.rollout_mock_length_mean
    sigma = rollout_config.rollout_mock_length_std
    np_rng = self._np_rng
    rng = self._rng
  ...
  def generate(
    ...
    for i, prompt in enumerate(prompts):
      target_length = self._get_target_length(rollout_config, max_tokens)
